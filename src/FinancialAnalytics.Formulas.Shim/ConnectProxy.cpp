...
static LPCWSTR szAddInAssemblyName = 
	L"FinancialAnalytics.Formulas, PublicKeyToken=null";
static LPCWSTR szConnectClassName = 
	L"FinancialAnalytics.Formulas.FormulaProcessor";
static LPCWSTR szAssemblyConfigName =
	L"FinancialAnalytics.Formulas.dll.config";

CConnectProxy::CConnectProxy() 
    : m_pConnect(NULL), m_pCLRLoader(NULL), m_pUnknownInner(NULL), m_pFormulasProcessor(NULL)
{
}
...
HRESULT CConnectProxy::FinalConstruct()
{
    ...
    IfFailGo(m_pConnect->QueryInterface(
              __uuidof(FinancialAnalytics_Formulas::IFormulaProcessor),   (void **)&m_pFormulasProcessor));
    ...
    ext_DisconnectMode RemoveMode, SAFEARRAY **custom)
{
    ...
    if (SUCCEEDED(hr))
    {
		...
		m_pFormulasProcessor->Release();
		m_pFormulasProcessor = NULL;
    }
...
//VARIANT __stdcall CConnectProxy::FA(VARIANT symbols, VARIANT dataItems, VARIANT options)
//{
//	return m_pFormulasProcessor->FA(symbols, dataItems, options);
//}
//
//HRESULT __stdcall CConnectProxy::raw_FA(VARIANT symbols, VARIANT dataItems, VARIANT options, VARIANT *resVal)
//{
//	return m_pFormulasProcessor->raw_FA(symbols, dataItems, options, resVal);
//}
