        ...
        public static QuoteProperty[] GetAllActiveProperties(IEnumerable<QuotesData> quotes)
        {
            ...
            if (quotes != null)
            {
                foreach (QuoteProperty qp in Enum.GetValues(typeof(QuoteProperty)))
                {
                    ...
                    {
                        var value = typeof (QuotesData).InvokeMember(qp.ToString(), BindingFlags.GetProperty, null, quote, null);
                        if (value != null)
                        {
                            valueIsNotNull = true;
            ...
            if (properties != null && properties.Length > 0)
            {
                foreach (QuoteProperty qp in properties)
                {
                    switch (qp)
                    {
                        ...
                        //    symbols.Append("b3");
                        //    break;
                        case QuoteProperty.BookValue:
                            symbols.Append("b4");
                            break;
                        //case QuoteProperty.BidSize:
                        //    symbols.Append("b6");
                        //    break;
                        case QuoteProperty.Change_PercentChange:
                            symbols.Append('c');
                            break;
